...
public final class GameFont extends DrawingArea {
	...
	/*
	 * Referred to as a 'glyph' rather than a 'character' as a character remains
	 * the same between fonts ('a' is always 'a') however a glyph is a
	 * representation of a character within a font.
	 * 
	 * http://graphicdesign.stackexchange.com/questions/13438/whats-the-
	 * practical-difference-between-a-glyph-and-a-character
	 */
	 ...
	 * 
	 * @param archive
	 *            The archive containing the information about the font.
	 * 
	 * @param monospace
	...
	public GameFont(String name, Archive archive, boolean monospace) {
		...
		/*
		 * The buffer containing data about this specific font. The position of
		 * the font within the parent archive (index.dat) and the pixel data for
		 * the glyph.
		 */
		...
		for (int g = 0; g < 256; g++) {
			...
			/*
			 * Set the pixels for the glyph based on whether it is square or
			 * rectangular.
			 */
			if (rectangular == 0) {
			...
			/*
			 * If the height of this glyph is higher than the highest glyph
			 * we've currently processed for this font, store this glyph's
			 * height as the highest.
			 * 
			 * 126 is the last visible character used in this client (~). No
		...
		/*
		 * Character 32 is space, character 73 is uppercase I and character 105
		 * is lowercase i.
		 */
		if (monospace) {
	...
	public void drawAlphaTextWithShadow(String text, int x, int y, int currentColour, int seed) {
		...
		/*
		 * Generate a random alpha value that is more opaque than the shadow
		 * (192 is the alpha value for the shadow).
		 */
		...
		for (int c = 0; c < text.length(); c++) {
			/*
			 * Check if the current piece of text being processed is a colour
			 * for (@dre@ for example)
			 */
			if (text.charAt(c) == '@' && c + 4 < text.length() && text.charAt(c + 4) == '@') {
				...
				/*
				 * If the character is a space, we don't draw it - we just add
				 * the width of the space to the current x position.
				 */
				if (character != ' ') {
	...
	public void drawCentredText(String text, int x, int y, int colour) {
		/*
		 * Draw the text with half of it to the left of the anchor, and half of
		 * it to the right of the anchor.
		 */
	...
	public void drawShakingText(String text, int x, int y, int colour, int elapsed, int tick) {
		...
		/*
		 * Calculate the current amplitude of the shake based on how long the
		 * text has been shaking for.
		 */
		...
		/*
		 * Place half the text to the left of the anchor point and half the text
		 * to the right. This centres the text horizontally.
		 */
		...
		for (int c = 0; c < text.length(); c++) {
			...
			/*
			 * If the character is a space we do not draw it, but we still move
			 * the x position by the width of the space.
			 */
			if (character != ' ') {
				/*
				 * The vertical position of the text is decided based on a sine
				 * wave, taking into account the current time and the amount of
				 * time the text has been shaking for.
				 */
	...
	public void drawText(String text, int x, int y, int colour) {
		...
		for (int c = 0; c < text.length(); c++) {
			...
			/*
			 * If the character is a space, we don't draw it - we just add the
			 * width of the space to the current x position.
			 */
			if (character != ' ') {
	 ...
	 * 
	 * Used for interface drawing where the text could be shadowed but also
	 * could be shadowless.
	 * 
	 * @param text
	 *            The text to draw.
	...
	public void drawTextWithPotentialShadow(String text, int x, int y, int currentColour, boolean shadowed) {
		...
		for (int c = 0; c < text.length(); c++) {
			...
			if (text.charAt(c) == '@' && c + 4 < text.length() && text.charAt(c + 4) == '@') {
				...
				/*
				 * If the character is a space, we don't need to draw it (but we
				 * do still offset the current x pointer using the glyph's
				 * width).
				 */
				if (character != ' ') {
					/*
					 * If the text has a shadow, we draw the shadow at a slight
					 * offset and then draw the normal coloured text on top of
					 * it.
					 */
					if (shadowed) {
		...
		/*
		 * If the current piece of text has a strikethrough applied, draw a
		 * horizontal line from the start of the text to the current position.
		 */
		if (strikethrough) {
	...
	/**
	 * Draw text that is waving based on a given tick. The text is drawn around
	 * a central anchor point. The text follows a sine wave vertically and
	 * horizontally.
	 * 
	 * @param text
	 *            The text to draw.
	...
	public void drawVerticalHorizontalSineWaveText(String text, int x, int y, int colour, int tick) {
		...
		for (int c = 0; c < text.length(); c++) {
			...
			if (character != ' ') {
				/*
				 * The x and y positions of the text are determined using sine
				 * waves.
				 */
	...
	/**
	 * Draw text that is waving based on a given tick. The text is drawn around
	 * a central anchor point. The text follows a sine wave vertically.
	 * 
	 * @param text
	 *            The text to draw.
	 * 
	...
	/**
	 * Get the displayed width of a text containing embedded colours, crowns
	 * etc.
	 * 
	 * @param text
	...
	public int getTextDisplayedWidth(String text) {
		...
		for (int c = 0; c < text.length(); c++) {
			/*
			 * If the current character is the start of a colour code or a crown
			 * code we can skip it as it will be stripped out later and
			...
			if (text.charAt(c) == '@' && c + 4 < text.length() && text.charAt(c + 4) == '@') {
				...
				/*
				 * Add the displayed width of the current character's glyph to
	...
	public int getTextWidth(String text) {
		...
		/*
		 * Iterate through every character in the text and add the displayed
		 * width of that character's glyph to the total width of the text.
		 */
		for (int c = 0; c < text.length(); c++) {
			width += glyphDisplayWidth[text.charAt(c)];
		}
	 ...
	 * 
	 * @return The numeric representation of the colour, or -1 if the colour
	 *         does not exist.
	 */
	 ...
	 * 
	 * @param input
	 *            The array of pixel states (1 if drawn, 0 if not drawn.)
	 * 
	 * @param inputPosition
	 *            The position in the input array to start at.
	 * 
	 * @param inputWidth
	 *            The amount to increase the position in the input array by
	 ...
	 * 
	 * @param outputWidth
	 *            The amount to increase the position in the output array by
	 *            after each iteration.
	 * 
	 * @param width
	...
	private void render(byte input[], int inputPosition, int inputWidth, int output[], int outputPosition,
		...
		for (int row = -height; row < 0; row++) {
			for (int col = _width; col < 0; col++) {
				/*
				 * If the pixel in the current position is set, draw it with a
				 * given colour. If not, simply leave a space.
				 */
	...
	private void renderAlpha(byte input[], int inputPosition, int inputWidth, int output[], int outputPosition,
		...
		for (int row = -height; row < 0; row++) {
			for (int col = -width; col < 0; col++) {
				/*
				 * If the pixel is set then draw it, if not simply leave a
				 * space.
				 */
				if (input[inputPosition++] != 0) {
